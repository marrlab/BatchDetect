# taken from MDMM repo, only changed naming (todo: revise)
...
class EncoderAttribute(nn.Module):
    def __init__(self, input_dim, output_nc=8, c_dim=3, norm_layer=None, nl_layer=None, no_domain=False):
        super(EncoderAttribute, self).__init__()
...
class Generator(nn.Module):
    def __init__(self, output_dim, c_dim=3, nz=8, no_domain=False):
...
class Discriminator(nn.Module):
    def __init__(self, input_dim, norm='None', sn=False, c_dim=3, image_size=216):
        ...
        self.conv1 = nn.Conv2d(curr_dim, 1, kernel_size=1,
                               stride=1, padding=0, bias=False)  # was padding=1 before -s
        kernel_size = int(image_size/np.power(2, n_layer))
...
class DiscriminatorContentLocalFeatures(nn.Module):
    def __init__(self, input_dim=64, c_dim=3):
        super(DiscriminatorContentLocalFeatures, self).__init__()
        model = []
        model += [LeakyReLUConv2d(input_dim, 64, kernel_size=5, stride=1, padding=0, norm='Instance')]
        model += [LeakyReLUConv2d(64, 64, kernel_size=3, stride=1, padding=0, norm='Instance')]
        model += [LeakyReLUConv2d(64, 64, kernel_size=3, stride=1, padding=0, norm='Instance')]
        model += [LeakyReLUConv2d(64, 64, kernel_size=3, stride=1, padding=0, norm='Instance')]
        model += [nn.AdaptiveAvgPool2d(1)]
        model += [nn.Conv2d(64, c_dim, kernel_size=1, stride=1, padding=0)]
        self.model = nn.Sequential(*model)
